...
report 50102 "Sales-Credit Memo Custom"
{
    Caption = 'Sales - Credit Memo';
    DefaultRenderingLayout = "StandardSalesCreditMemoNew.docx";
    ...
    dataset
    {
        dataitem(Header; "Sales Cr.Memo Header")
        {
            ...
            dataitem(Line; "Sales Cr.Memo Line")
            {
                ...
                column(DimCodeValue1; DimCodeValue[1])
                {
                }
                column(DimCodeValue2; DimCodeValue[2])
                {
                }
                column(DimCodeValue3; DimCodeValue[3])
                {
                }
                column(DimCodeValue4; DimCodeValue[4])
                {
                }
                column(DimCodeValue5; DimCodeValue[5])
                {
                }
                column(DimCodeValue6; DimCodeValue[6])
                {
                }
                column(DimCodeValue7; DimCodeValue[7])
                {
                }
                column(DimCodeValue8; DimCodeValue[8])
                {
                }
                ...
                var
                    DimManagment: Codeunit DimensionManagement;
                    GLSetup: Record "General Ledger Setup";
                    DimSetEntries: Record "Dimension Set Entry";
                    i: Integer;
                begin
                    ...
                    Clear(DimCode);
                    Clear(DimCodeValue);
                    DimManagment.GetGLSetup(DimCode);
                    for i := 1 to 8 do begin
                        if DimCode[i] <> '' then begin
                            DimSetEntries.Reset();
                            DimSetEntries.SetRange("Dimension Set ID", "Dimension Set ID");
                            DimSetEntries.SetRange("Dimension Code", DimCode[i]);
                            if DimSetEntries.FindFirst() then
                                DimCodeValue[i] := DimSetEntries."Dimension Value Code"
                            else
                                DimCodeValue[i] := '';
                        end;
    ...
    rendering
    {
        ...
        // }
        layout("StandardSalesCreditMemoNew.docx")
        {
            Type = Word;
            LayoutFile = './SRC/Layout/StandardSalesCreditMemoNew.docx';
...
#pragma warning restore AA0470
        ...
        DimCodeValue: array[8] of Code[20];
